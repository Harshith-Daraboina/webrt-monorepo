...
io.on('connection', (socket) => {
  ...
  socket.on('join-room', async (data: { roomId: string; userId: string }) => {
    try {
      ...
      try {
        logger.info(`💾 Saving Room ${roomId} and Session for User ${userId} to database...`);
        ...
        await prisma.user.upsert({
          ...
          // User might already exist or creation failed - continue
        });
        
        // Try to find user by name if email upsert failed
        ...
        if (!dbUser) {
          ...
          logger.info(`✅ User created in database: ${dbUser.id}`);
        }
        ...
        if (!room) {
          ...
          logger.info(`✅ Room created in database: ${room.id}`);
        } else {
          // Update room status
          ...
          logger.info(`✅ Room updated in database: ${room.id}`);
        }
        
        // Now create/update session using dbUser.id
        ...
        if (existingSession) {
          ...
          logger.info(`✅ Session updated in database: ${session.id}`);
        } else {
          // Create new session (id will be auto-generated by MongoDB)
        ...
        try {
          ...
          logger.error('❌ Cannot connect to database:', connError);
        }
  ...
  // Handle WebRTC signaling - send to specific peer or broadcast to all
  socket.on('offer', (data: { roomId: string; offer: RTCSessionDescriptionInit; targetPeerId?: string }) => {
    const roomUsers = roomMembers.get(data.roomId);
    const sender = roomUsers?.get(socket.id);
    const userId = sender?.userId || 'unknown';
    const peerId = sender?.peerId || 'unknown';
    
    logger.info(`📤 Offer from ${socket.id} (${userId}) in room ${data.roomId}`);
    
    // If targetPeerId is specified, send only to that peer
    if (data.targetPeerId) {
      // Find the socket ID of the target peer
      const targetSocketId = Array.from(roomUsers?.entries() || []).find(
        ([_, user]) => user.peerId === data.targetPeerId
      )?.[0];
      ...
      if (targetSocketId) {
        ...
        logger.info(`  → Sent to specific peer: ${targetSocketId}`);
      } else {
        logger.warn(`  ⚠️ Target peer ${data.targetPeerId} not found, broadcasting to all`);
      ...
      // Broadcast to all in room (legacy behavior)
      socket.to(data.roomId).emit('offer', {
        offer: data.offer,
        from: socket.id,
        userId: userId,
      ...
      logger.info(`  → Broadcasted to all in room`);
    }
  });
  ...
  socket.on('answer', (data: { roomId: string; answer: RTCSessionDescriptionInit; targetPeerId?: string }) => {
    ...
    logger.info(`📤 Answer from ${socket.id} (${userId}) in room ${data.roomId}`);
...
const NETWORK_IP = '10.0.5.43';

httpServer.listen(PORT, HOST, () => {
  logger.info(`✅ Signaling server running on ${HOST}:${PORT}`);
  logger.info(`   - Local:    http://localhost:${PORT}`);
  logger.info(`   - Network:  http://${NETWORK_IP}:${PORT}`);
});
